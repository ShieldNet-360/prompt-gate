## Summary

A short description of the change.

## Motivation

Why is this change needed? Link to an issue or design discussion if
one exists.

## Changes

- Bullet list of the concrete things this PR does.

## Checklist

- [ ] I have read [CONTRIBUTING.md](../CONTRIBUTING.md).
- [ ] I have added or updated unit tests for the new behaviour.
- [ ] I have updated `CHANGELOG.md` if the change is user-visible.
- [ ] I have updated the relevant `.md` documentation (`README.md`,
      `ARCHITECTURE.md`, etc.) when behaviour changes.
- [ ] CI is green locally (`make test && make lint` in `agent/`,
      `npm run typecheck` in `electron/` and `extension/`,
      `npm test` in `extension/`).
- [ ] **Privacy invariant**: I confirm this change does not persist
      scanned content, domain names, URLs, or IP addresses to disk
      and does not transmit them off-device.

## Screenshots / output

(Optional) For UI changes, paste a screenshot of the new state. For
new patterns, paste an example match transcript.
